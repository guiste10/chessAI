...
class Board:
    ...
    def add_pawn_moves(self, pawn, is_white, moves):
        ...
        if target_row[idx] == target_row[idx + 3]:  # promotion on next row
            ...
            if is_enemy[is_white](self.board[target_row[idx]][col - 1]):
                ...
                moves["capture"].append(Move(row, col, target_row[idx], col - 1, to_piece))
            if is_enemy[is_white](self.board[target_row[idx]][col + 1]):
                to_piece = self.board[target_row[idx]][col + 1]
                moves["capture"].append(Move(row, col, target_row[idx], col + 1, to_piece))
    ...
    def add_rook_moves(self, rook, is_white, moves):
        self.add_directed_rook_moves(rook, is_white, moves)

    def add_directed_rook_moves(self, rook, is_white, moves):
        ...
        if self.cannot_castle[is_white] or (row, col) not in rook_start_pos[is_white] or self.rook_moved[(row, col)]:
            ...
            for direction in rook_directions:
                ...
                while self.board[to_row][to_col] == Pieces.OO:
                    moves["move"].append(MoveCastlingRightsChange(row, col, to_row, to_col, is_white))
                    to_row, to_col = to_row + direction[0], to_col + direction[1]
                if is_enemy[is_white](self.board[to_row][to_col]):
                    moves["capture"].append(MoveCastlingRightsChange(row, col, to_row, to_col, is_white, self.board[to_row][to_col]))
    ...
    def add_directed_moves(self, piece, directions, is_white, moves):
        ...
        for direction in directions:
            ...
            while self.board[to_row][to_col] == Pieces.OO:
                ...
                to_row, to_col = to_row + direction[0], to_col + direction[1]
            if is_enemy[is_white](self.board[to_row][to_col]):
                moves["capture"].append(Move(row, col, to_row, to_col, self.board[to_row][to_col]))
    ...
    def add_king_square_moves(self, row, col, to_squares, is_white, moves):
        if self.cannot_castle[is_white]:
            for to_square in to_squares:
                to_square_val = self.board[to_square[0]][to_square[1]]
                if to_square_val == Pieces.OO:
                    moves["move"].append(KingMove(row, col, to_square[0], to_square[1], is_white))
                elif is_enemy[is_white](to_square_val):
                    moves["capture"].append(KingMove(row, col, to_square[0], to_square[1], is_white, to_square_val))
        else:
            for to_square in to_squares:
                to_square_val = self.board[to_square[0]][to_square[1]]
                if to_square_val == Pieces.OO:
